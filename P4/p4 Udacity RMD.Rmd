...
```{r echo=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
#It is more helpful to view charts in an ordered way, so let's put the 
#ordering and subsetting code into a function (we use it alot).
chart_order_func <- function(data, x, number){
  data_new <- within(data, data[[x]] <- 
                       factor(data[[x]], 
...
Let's first look at the number of contributions across cities. Given the skew, we will log transform the data to get a better understanding of the distribution.
...
qplot(x=data$contbr_city, data=data) +
  ...
  scale_y_log10() +
  labs(x="Cities in Texas ordered by number of contributions", 
       y='Count (log)')
...
Now let's move on to another variable and plot contributions by occupation.  Again we will log transform.
...
qplot(x=data$contbr_occupation, data=data) +
  scale_x_discrete(breaks=NULL) +
  scale_y_log10() +
  labs(x="Occupation ordered by number of contributions per")
...
Next let's look at contributions by candidate.  Here we will log transform as a few candidates have orders of magnitude more contributions than others.
...
qplot(x=cont$contb_receipt_amt, data=cont, binwidth=100) +
  scale_x_continuous(limits=c(-3000,12500), 
                     breaks=round(seq(-3000, 12500, by=250),1)) + 
...
leading_cand <- c("Clinton, Hillary Rodham", "Cruz, Rafael Edward 'Ted'", 
                  "Rubio, Marco", "Sanders, Bernard", "Trump, Donald J.")

top5cities <- c("HOUSTON", "SAN ANTONIO", "DALLAS", "AUSTIN", "FORT WORTH")
top11occupations <- c("RETIRED", "INFORMATION REQUESTED BEST EFFORTS", 
                      "HOMEMAKER", "NOT EMPLOYED", "ATTORNEY", "PHYSICIAN", 
                      "ENGINEER", "SALES", "CONSULTANT", "OWNER", "TEACHER")

cont_df <- data.frame(cont)

cont_df_leading <- cont_df[cont_df$cand_nm %in% leading_cand,]
cont_df_leading_city <- 
  cont_df_leading[cont_df_leading$contbr_city %in% top5cities,]
cont_df_leading_occupation <- 
  cont_df_leading[cont_df_leading$contbr_occupation %in% top11occupations,]
...
cont_df_fortworth <- 
  cont_df_leading[cont_df_leading$contbr_city=='FORT WORTH',]
cont_df_sanantonio <- 
  cont_df_leading[cont_df_leading$contbr_city=='SAN ANTONIO',]
...
cont_df_leading_cityandoccupation <- 
  cont_df_leading_city[cont_df_leading_city$contbr_occupation 
                       %in% top11occupations,]
...
qplot(x=datefield, y=contb_receipt_amt, data=cont_df_leading) +
  scale_x_date(labels=date_format("%b-%d-%Y"), 
               breaks = seq(as.Date("2015-01-01"), 
                            as.Date(max(cont_df_leading$datefield)), 
                            by="1 month"), 
               limits = c(as.Date("2015-01-01"), 
                          as.Date(max(cont_df_leading$datefield)))) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
...
data <- chart_order_func(cont_df_leading_occupation, "contb_receipt_amt", 0)
ggplot(data, aes(x=contb_receipt_amt)) +
  geom_histogram(aes(fill = contbr_occupation), 
                 position = 'fill', binwidth = 100) +
  scale_x_continuous(limits=c(0,3000), 
                     breaks=round(seq(0, 3000, by=100),1)) +
  theme(axis.text.x = element_text(angle=60, hjust=1))
...
data <- chart_order_func(cont_df_leading_cityandoccupation, 
                         "contbr_occupation", 0)
...
ggplot(aes(x=cand_nm,y=contb_receipt_amt), 
       data=cont_df_leading_cityandoccupation) +
  geom_boxplot(aes(color = contbr_occupation)) + scale_y_log10() + 
  scale_color_hue() + theme(axis.text.x = element_text(angle=60, hjust=1))
...
ggplot(aes(x=contbr_occupation,y=contb_receipt_amt), 
       data=cont_df_leading_cityandoccupation) +
  geom_boxplot(aes(color = contbr_city)) + scale_y_log10() + 
  scale_color_hue() + theme(axis.text.x = element_text(angle=60, hjust=1))
...
ggplot(aes(x=cand_nm,y=contb_receipt_amt), 
  ...
  ggtitle("Contribution Size Ranges per Candidate Split by City") +
  labs(x="Leading Presidential Candidates", y="Contribution Size (Log)")
